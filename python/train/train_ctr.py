...
# 3.5) Feature selection with mRMR 
...
search = RandomizedSearchCV(
    ...
    cv=3,
    verbose=2,
    n_jobs=1,   # safer for Windows/Python 3.13
    random_state=42
)
...
# ========= Complexity via permutation importance + polynomial fits =========

rng = np.random.default_rng(42)
idx = rng.choice(X_test_s.shape[0], size=min(2000, X_test_s.shape[0]), replace=False)
X_ref = pd.DataFrame(X_test_s[idx], columns=selected_feats)
f = best_model.predict_proba(X_ref.values)[:, 1]
...
used_feats = list(np.array(selected_feats)[imp > thr])
...
IAS_pairs = {f"{a}×{b}": ias_pair(a, b) for a, b in pairs if a in X_ref.columns and b in X_ref.columns}
...
def table_row_for_model(model, model_name, mae, complexity, round_to=2):
    ...
    mec_total = float(np.nansum(list(complexity["MEC"].values())))
